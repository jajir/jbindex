...
public class TypeConvertors {
	...
	static {
		...
		}
		{
			...
			instance.add(String.class, OperationType.CONVERTOR_FROM_BYTES, tds.getConvertorFromBytes());
		...
		}
		{
			...
			instance.add(Byte.class, OperationType.CONVERTOR_FROM_BYTES, tds.getConvertorFromBytes());
	...
	public void add(final Class<?> clazz, final OperationType operationType, final Object convertor) {
	...
	public <T> T get(final Class<?> clazz, final OperationType operationType) {
		...
		if (out == null) {
			throw new NullPointerException(String.format("There is no %s for key %s", operationType, key));
		}
		return out;
	}
