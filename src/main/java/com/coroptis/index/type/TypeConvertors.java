...
public class TypeConvertors {
	...
	final T out = (T) convertors.get(makeKey(clazz, operationType));
	if (out == null) {
	    throw new NullPointerException(
		    String.format("There is no %s for class %s", operationType, clazz.getName()));
	}
	return out;
    }

    private String makeKey(final Class<?> clazz, final OperationType operationType) {
	return clazz.getName() + SEPARATOR + operationType.name();
    }
