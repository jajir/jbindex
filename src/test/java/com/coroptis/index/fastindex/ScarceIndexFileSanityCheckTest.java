...
public class ScarceIndexFileSanityCheckTest {
    ...
    public void test_sanityCheck() throws Exception {
        final SortedDataFile<String,Integer> sdf = new SortedDataFile<>(directory, "index.map",
                integerTd.getTypeWriter(), integerTd.getTypeReader(),
                stringTd.getComparator(),
                stringTd.getConvertorFromBytes(),
                stringTd.getConvertorToBytes());


        try (final SortedDataFileWriter<String, Integer> writer = sdf.openWriter()) {
        ...
        assertThrows(IllegalStateException.class, ()->{  try (final ScarceIndexFile<String> fif = new ScarceIndexFile<>(directory,
        stringTd)) {
